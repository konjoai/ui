...
_STRIPPERS = {".ts": _strip_c_comments, ".tsx": _strip_c_comments, ".js": _strip_c_comments, ".jsx": _strip_c_comments}
...
SUPPORTED_EXTENSIONS = {".ts", ".tsx", ".js", ".jsx", ".rs", ".py"}
...
def _iter_sources(root: Path, extensions: set[str]) -> Iterator[Path]:
    for path in root.rglob("*"):
        ...
        if path.suffix in extensions and path.is_file(): yield path
...
def find_duplicates(files: list[Path], all_files: list[Path], threshold: float, min_lines: int) -> list[dict]:
    ...
    for path in all_files:
        lines = _normalize_lines(path); file_lines[path] = lines
    ...
    for target_path in files:
        ...
        for digest, start in _window_fingerprints(lines, min_lines):
            for other_path, other_start, other_lines in index.get(digest, []):
                if other_path == target_path and other_start == start: continue
                ...
                if sim >= threshold:
                    violations.append({"file_a": str(target_path), "line_a": start+1, "file_b": str(other_path), "line_b": other_start+1, "similarity": round(sim,3), "lines": min_lines, "sample": "\n".join(lines[start:start+5])})
    return violations

def main() -> int:
    p = argparse.ArgumentParser()
    p.add_argument("--root", default=None); p.add_argument("--threshold", type=float, default=0.85)
    p.add_argument("--min-lines", type=int, default=10)
    m = p.add_mutually_exclusive_group()
    m.add_argument("--staged-only", action="store_true"); m.add_argument("--changed-only", action="store_true")
    p.add_argument("--json", action="store_true", dest="json_out"); p.add_argument("--report")
    p.add_argument("--warn-only", action="store_true")
    p.add_argument("--extensions", default=",".join(sorted(SUPPORTED_EXTENSIONS)))
    args = p.parse_args()
    if args.root: root = Path(args.root)
    else:
    ...
    if args.staged_only: scan_targets = _staged_files(root, extensions)
    elif args.changed_only: scan_targets = _changed_files(root, extensions)
    else: scan_targets = all_files
    if not scan_targets:
        if not args.json_out: print("[dry-check] No files to check.")
        return 0
    ...
    if args.json_out: print(json.dumps(report, indent=2))
    else:
        if violations:
            print(f"[dry-check] ❌ {len(violations)} DRY violation(s) found")
            for v in violations: print(f"  {v['file_a']}:{v['line_a']} ↔ {v['file_b']}:{v['line_b']} ({v['similarity']*100:.0f}%)")
        else: print(f"[dry-check] ✅ No DRY violations ({len(scan_targets)} files scanned).")
